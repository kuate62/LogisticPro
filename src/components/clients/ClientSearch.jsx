...
export default function ClientSearch({ value, onChange }) {
  ...
  return (
    <div className="position-relative" style={{ maxWidth: 380 }}>
      ...
      <input type="text" className="form-control ps-5 pe-5 py-2" placeholder="Nom, prénom, téléphone, code client..." value={local} onChange={handleChange} />
      {local && (
        <button type="button" className="btn btn-sm position-absolute top-50 end-0 translate-middle-y me-2 p-0 border-0 bg-transparent" onClick={() => { setLocal(''); onChange(''); }}>
