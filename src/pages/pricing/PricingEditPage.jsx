...
import { pricingFormSchema, pricingToFormValues, pricingFormValuesToPayload } from '../../helpers/pricingValidation';
...
export default function PricingEditPage() {
  const { id } = useParams();
  const { pricing, loading, fetch, clearSelected } = usePricing();
  const { update } = usePricingForm();
  ...
  useEffect(() => {
    if (pricing) {
      reset(pricingToFormValues(pricing));
    }
  }, [pricing, reset]);
  ...
  const onSubmit = async (values) => {
    try {
      ...
      toast.error('Erreur lors de la modification du tarif');
    }
  };
  ...
  return (
    <div>
      <div className="d-flex align-items-center gap-3 mb-4">
        ...
        <div>
          <h4 className="fw-bold text-dark mb-1">Modifier le tarif</h4>
          <p className="text-muted mb-0 small">{pricing.name} — {pricing.code}</p>
        </div>
      </div>
